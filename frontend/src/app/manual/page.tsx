...
const FEATURES: Feature[] = [
  ...
  },
  {
    menu: "문서 Q&A",
    icon: "?",
    what: "수집 문서를 근거로 자연어 질문에 답한다. 답변은 근거가 된 [문서 N]을 인용하고, 사용한 문서를 함께 보여준다.",
    how: "질문을 입력하고 ‘질문’. 문서에 없는 내용은 ‘확인되지 않음’으로 답한다.",
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-6">
        {/* 개요 */}
        <Card>
          <h2 className="text-base font-semibold text-zinc-50">서비스 개요</h2>
          <p className="mt-2 text-sm leading-relaxed text-zinc-300">
            <strong className="text-zinc-100">MI Report Agent</strong>는 반도체·IT 시장
            인텔리전스(MI) 리포트 작성을 자동화하는 에이전트다. 시장 센싱 · 뉴스
            다이제스트 · 경쟁사 IR 분석을 한 곳에서 다룬다. 핵심 흐름은 다음 한 줄로
            요약된다:
          </p>
          <p className="mt-3 rounded-lg border border-sky-900/40 bg-sky-950/30 px-4 py-3 text-sm font-medium text-sky-200">
        ...
        {/* 구성 */}
        <Card>
          <h2 className="text-base font-semibold text-zinc-50">구성</h2>
          <div className="mt-3 grid gap-3 sm:grid-cols-3">
            <div className="rounded-lg bg-zinc-800/50 px-4 py-3">
              <p className="text-xs font-medium text-zinc-400">프론트엔드</p>
              <p className="mt-1 text-sm text-zinc-200">Next.js 대시보드</p>
              <p className="mt-1 text-xs text-zinc-500">localhost:3000</p>
            </div>
            <div className="rounded-lg bg-zinc-800/50 px-4 py-3">
              <p className="text-xs font-medium text-zinc-400">백엔드</p>
              <p className="mt-1 text-sm text-zinc-200">FastAPI</p>
        ...
        {/* 기능별 사용법 */}
        <section>
          <h2 className="mb-3 text-base font-semibold text-zinc-50">화면별 사용법</h2>
              ...
              <Card key={f.menu}>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-semibold text-zinc-100">{f.menu}</h3>
                  </div>
                  <div className="flex items-center gap-2">
                    {f.live && (
                      <span className="rounded-full border border-emerald-900/60 bg-emerald-950/40 px-2.5 py-0.5 text-[11px] text-emerald-400">
                        실연동
                      </span>
                    )}
                ...
                <p className="mt-2 text-sm leading-relaxed text-zinc-300">{f.what}</p>
        ...
        {/* 권장 워크플로우 */}
        <Card>
          <h2 className="text-base font-semibold text-zinc-50">권장 워크플로우</h2>
          <ol className="mt-3 flex flex-col gap-2.5">
            {WORKFLOW.map((step, i) => (
              <li key={i} className="flex gap-3 text-sm leading-relaxed text-zinc-300">
                <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-sky-900/60 text-[11px] font-medium text-sky-300">
                  {i + 1}
                </span>
                {step}
              </li>
        ...
        <Card className="border-amber-900/50 bg-amber-950/20">
          <h2 className="text-base font-semibold text-amber-200">알아둘 점</h2>
          <ul className="mt-3 flex flex-col gap-2 text-sm leading-relaxed text-zinc-300">
            <li>
              · <strong className="text-zinc-100">근거 기반 생성</strong> — 생성물은 제공된
              문서에만 근거한다. 문서가 없으면 빈 상태로 안내하며 사실을 지어내지 않는다.
            </li>
            <li>
              · <strong className="text-zinc-100">검토 후 활용</strong> — 단일 출처 항목은
