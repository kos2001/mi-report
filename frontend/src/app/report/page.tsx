    ...
    <>
      ...
      <Card className="mb-8">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">AI 주간 리포트 생성</h2>
          ...
          {showTemplate && (
            <div className="mt-2">
              <textarea
                ...
                rows={9}
                className="w-full rounded-lg border border-zinc-300 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-900 px-3 py-2 font-mono text-xs text-zinc-900 dark:text-zinc-100 outline-none focus:border-sky-500"
              />
              <p className="mt-1 text-[11px] text-zinc-500">
                토큰: <code className="text-zinc-600 dark:text-zinc-400">{"{{issue_no}} {{period}} {{generated_at}} {{overview}} {{priority_risk}} {{critical_points}} {{digest}} {{topics}}"}</code>
              </p>
            </div>
          )}
        </div>
        ...
        <Card className="mb-8">
          <div className="flex items-center justify-between gap-4">
            <h2 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
              문서 미리보기 — <span className="font-mono text-xs text-zinc-500">{docPreview.filename}</span>
            </h2>
            <div className="flex shrink-0 items-center gap-2">
              <button
                onClick={() => downloadMarkdown(docPreview.filename, docPreview.markdown)}
                className="rounded-lg bg-sky-600 px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-sky-500"
              >
                다운로드
              </button>
              <button
                onClick={() => setDocPreview(null)}
                className="rounded-lg border border-zinc-300 dark:border-zinc-700 bg-zinc-200 dark:bg-zinc-800 px-3 py-1.5 text-xs text-zinc-700 dark:text-zinc-300 transition-colors hover:bg-zinc-300 dark:hover:bg-zinc-700"
              >
                닫기
              </button>
            </div>
          </div>
          <div className="mt-3 max-h-[70vh] overflow-y-auto rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 px-4 py-3">
            <Markdown text={docPreview.markdown} className="text-sm text-zinc-700 dark:text-zinc-300" />
          </div>
        </Card>
      )}
      ...
      {report && (
        <div className="flex flex-col gap-6">
          ...
          {/* 총평 */}
          <Card className="border-sky-100/50 dark:border-sky-900/50 bg-sky-50/20 dark:bg-sky-950/20">
          ...
          {((report.priorities && report.priorities.length > 0) ||
            ...
            <section>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {report.priorities && report.priorities.length > 0 && (
                  <Card>
                    <h4 className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">Priority</h4>
                    <ol className="mt-2 flex flex-col gap-2 text-sm text-zinc-700 dark:text-zinc-300">
                      {report.priorities.map((p, i) => (
                        <li key={i}>
                          <span className="font-medium text-zinc-900 dark:text-zinc-100">{p.rank}. {p.title}</span>
                          {!p.evidenceGrounded && <span className="ml-1 text-amber-600 dark:text-amber-400">⚠ 근거 미확인</span>}
                          <p className="text-xs text-zinc-600 dark:text-zinc-400">{p.rationale}</p>
                        </li>
                      ))}
                    </ol>
                  </Card>
                ...
                {report.risks && report.risks.length > 0 && (
                  <Card>
                    <h4 className="text-sm font-semibold text-red-600 dark:text-red-400">Risk</h4>
                    <ol className="mt-2 flex flex-col gap-2 text-sm text-zinc-700 dark:text-zinc-300">
                      {report.risks.map((r, i) => (
                        <li key={i}>
                          <span className="font-medium text-zinc-900 dark:text-zinc-100">{r.rank}. {r.title}</span>
                          {!r.evidenceGrounded && <span className="ml-1 text-amber-600 dark:text-amber-400">⚠ 근거 미확인</span>}
                          <p className="text-xs text-zinc-600 dark:text-zinc-400">{r.rationale}</p>
                        </li>
                      ))}
                    </ol>
                  </Card>
                ...
                {report.criticalPoints.map((c, i) => (
                  <Card key={i}>
                    <h4 className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
                      {c.title}
                      {!c.evidenceGrounded && <span className="ml-1 text-amber-600 dark:text-amber-400">⚠ 근거 미확인</span>}
                    </h4>
                    <dl className="mt-2 flex flex-col gap-1 text-xs text-zinc-600 dark:text-zinc-400">
                      {c.rootCause && <div><dt className="inline font-medium text-zinc-700 dark:text-zinc-300">근본원인: </dt>{c.rootCause}</div>}
                      {c.chainEffect && <div><dt className="inline font-medium text-zinc-700 dark:text-zinc-300">연쇄효과: </dt>{c.chainEffect}</div>}
                      {c.decisionNeeded && <div><dt className="inline font-medium text-zinc-700 dark:text-zinc-300">필요한 결정: </dt>{c.decisionNeeded}</div>}
                    </dl>
                  </Card>
                ))}
              </div>
                ...
                {report.topics.map((t) => (
                  <Card key={t.id}>
                    <div className="flex items-center gap-2">
                      <Tag>{t.category}</Tag>
                      <h4 className="text-sm font-semibold text-zinc-950 dark:text-zinc-50">{t.title}</h4>
                    </div>
