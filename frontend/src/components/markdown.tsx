...
function renderInline(text: string): ReactNode[] {
  ...
  while ((m = re.exec(text)) !== null) {
    ...
    const tok = m[0];
    if (tok.startsWith("**")) {
      nodes.push(<strong key={key++} className="font-semibold text-zinc-100">{tok.slice(2, -2)}</strong>);
    } else {
      nodes.push(
    ...
    if (h) {
      ...
      const cls =
        level === 1
          ...
          : level === 2
            ? "mt-3 mb-1 text-sm font-semibold text-zinc-100"
            : "mt-2 mb-1 text-sm font-medium text-zinc-200";
