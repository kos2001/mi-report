...
**관점 라벨을 붙이는 것도 지적 대상이 아니다.** 원문에 실재하는 사실 앞에 "S.LSI/SET
관점에서", "반도체 시황상" 처럼 어느 관점·범주에서 본 사실인지 표시하는 접두어를 붙이는
것은 새로운 사실 추가가 아니라 문서화 방식이다. 예를 들어 원문에 "주제별 History 자동
갱신"이 있다면, 초안이 이를 "S.LSI/SET 주제별 History 자동 갱신"으로 서술해도 원문
사실은 그대로이므로 지적하지 마라 — 관점 라벨이 사실 자체를 바꾸거나 새 수치·주체를
추가한 게 아니라면 정상이다.
