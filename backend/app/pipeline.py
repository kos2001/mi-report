...
→ 본문 추출 → 문서 저장)과 다이제스트 생성을 한 번에 돌리고, 산출물을 생성물
저장소(assets)에 영속화해 나중에 조회·삭제할 수 있게 한다(대화형 생성과 동일한
저장소 — 이력·삭제가 생성 경로와 무관하게 항상 동작한다). 게이트웨이가 떠 있어야
다이제스트가 생성된다.
"""
...
async def run_digest(*, period: str, limit: int = 20) -> dict[str, Any]:
    """수집 문서로 다이제스트를 생성하고 생성물(artifacts)로 저장한다.

    대화형 생성(/digest/generate)과 같은 저장소(assets)를 쓴다 — 스케줄/수동 실행
    산출물도 생성 이력에 나타나고 삭제할 수 있어야 하기 때문(별도 파일 저장소를
    쓰면 이력에 안 보이고 지울 수도 없는 사각지대가 생긴다)."""
...
def load_latest_digest() -> dict[str, Any] | None:
    """가장 최근에 생성된(아직 삭제되지 않은) 다이제스트 생성물. 없으면 None."""
    rows = assets.list_artifacts(kind="digest", limit=1)
    if not rows:
        return None
    return assets.get_artifact(rows[0]["id"])["payload"]
