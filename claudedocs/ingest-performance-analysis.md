...
**VLM 캡셔닝 (2026-07-07 구현)**
- OCR 로 글자가 거의 안 나오는 이미지(차트·다이어그램)는 OpenRouter 비전 모델로
  내용을 한국어 요약시킨다. 기본 모델은 레포 기본 LLM 인 `minimax/minimax-m3`
  (이미지 입력 지원 확인, `MI_VLM_MODEL` 로 교체 가능).
- 진입점은 `imagetext.image_text()` 하나: OCR 우선(무료·로컬) → 40자 미만이면
  VLM 캡션으로 보완·결합. OCR 충분 시 VLM 미호출(비용 절약).
- **opt-in**: `MI_VLM=1` + `OPENROUTER_API_KEY`. 이미지가 외부 API 로 전송되므로
  기본 꺼짐. (사용자 결정 2026-07-07: DRM 문서 이미지의 외부 전송 허용 — 소스별
  차단 없이 전역 스위치만 둔다.)
- 실검증: 합성 막대 차트를 M3 가 "Q1→Q4 상승 추세"로 정확히 요약(한국어).

**한계와 후속 옵션**
1. **한국어 OCR**: RapidOCR 기본 인식 모델은 중·영문. 한국어 스캔본 품질이 필요하면
   PP-OCR korean 인식 모델(.onnx)을 받아 `MI_OCR_REC_MODEL` 로 지정(워커 배포에 포함).
   VLM 활성 시에는 M3 가 한국어 이미지도 읽으므로 보완된다.
2. **DRM 문서**: COM 폴백 경로는 텍스트만 추출한다(이미지 미회수). 필요 시 Word COM
   `SaveAs2`(filtered HTML)로 이미지를 내보내 OCR/VLM 하는 확장 여지 — DRM 정책상
   내보내기가 허용되는지 벤더 확인 선행.
3. **텍스트 있는 페이지의 삽입 이미지(PDF)**: 노이즈·비용 때문에 미적용. 필요 시
   페이지 내 이미지 블록만 선별 처리하는 옵션 추가 가능.
